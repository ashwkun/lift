 ...
 * because the thing being chosen is not visual. This one is. "Dark" and "Light"
 * describe the two palettes in `theme/tokens.ts` about as well as a colour name
 * describes a paint chip, and the app already owns both palettes as data, so it
 ...
 * or `accent` in the tokens and these three tiles change with it, so the preview
 * cannot drift into advertising a theme the app no longer has.
 */
      ...
      <View style={[styles.ring, { borderColor: selected ? colors.accent : colors.surface }]}>
        {preference === 'system' ? (
          <View style={styles.split}>
            {/* Both palettes, split down the middle. The seam is the point:
                "System" is not a third theme, it is the other two deferred to
                whichever one the phone is in. */}
            <Swatch palette={lightPalette} style={styles.half} />
            <Swatch palette={darkPalette} style={styles.half} />
          </View>
        ) : (
          <Swatch palette={preference === 'dark' ? darkPalette : lightPalette} />
        )}
      </View>
...
const styles = StyleSheet.create({
  picker: { flexDirection: 'row', gap: spacing.md },
  ...
  tile: { flex: 1, gap: spacing.sm, borderRadius: radius.md, padding: spacing.xs },
  ring: {
