 ...
 *
 * The named palettes in `theme/palettes.ts` are deliberately not represented
 * here, and cannot be: there are eight themes and this file has one bit of
 * information about which is wanted. Someone on Solarized with a dark OS gets
 * one dark frame before their cream canvas arrives. Closing that would mean
 * mirroring the preference into `localStorage`: a second source of truth for
 * a setting SQLite already owns, to save one frame, so it is left open. The
 * four dark ports all have near-black canvases, so for them the shell is
 * already very close to right.
 *
